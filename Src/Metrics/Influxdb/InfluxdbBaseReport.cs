...
namespace Metrics.Influxdb
{
	...
	/// </summary>
	internal class UdpInfluxdbReport : DefaultInfluxdbReport
	{
		/// <summary>
		/// Creates a new InfluxDB report that uses the Line Protocol syntax over UDP.
		/// </summary>
		/// <param name="influxDbUri">The URI of the InfluxDB server, including any query string parameters.</param>
		public UdpInfluxdbReport(Uri influxDbUri)
			: base(influxDbUri) {
		}
		...
		/// </summary>
		/// <param name="config">The InfluxDB configuration object.</param>
		public UdpInfluxdbReport(InfluxConfig config = null)
			: base(config) {
		}

		protected override InfluxConfig GetDefaultConfig(InfluxConfig defaultConfig) {
			var config = base.GetDefaultConfig(defaultConfig) ?? new InfluxConfig();
			//config.Writer = config.Writer ?? new InfluxdbHttpWriter(config); // TODO: implement UDP writer
			return config;
